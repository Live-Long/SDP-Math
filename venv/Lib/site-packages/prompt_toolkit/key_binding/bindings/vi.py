...
from prompt_toolkit.filters.app import vi_navigation_mode, vi_insert_mode, vi_insert_multiple_mode, vi_replace_mode, \
    vi_selection_mode, vi_waiting_for_text_object_mode, vi_digraph_mode, vi_mode, in_paste_mode, is_multiline, \
    vi_search_direction_reversed, vi_recording_macro
    ...
    def text_object_decorator(*keys, **kw):
        ...
        def decorator(text_object_func):
            ...
            if not no_move_handler:
                @key_bindings.add(*keys, filter=~vi_waiting_for_text_object_mode & filter & vi_navigation_mode,
                                  eager=eager)
                def _(event):
            ...
            if not no_selection_handler:
                @key_bindings.add(*keys, filter=~vi_waiting_for_text_object_mode & filter & vi_selection_mode,
                                  eager=eager)
                def _(event):
...
def load_vi_bindings():
    ...
    def _(event):
        ...
        if event._arg:
            ...
            return TextObject(d.translate_row_col_to_index(event.arg - 1, 0) - d.cursor_position,
                              type=TextObjectType.LINEWISE)
        else:
