...
class Draw_options:
    def __init__(self):
        ...
        self.info = ["Speed: ", "Volume: ", "Submerged height: ", "Submerged volume: ", "Gravity force: ", "Buoyancy: ",
                     "Drag: ", "Acceleration: "]
    ...
    def details(self, surface, calculations, radius, scale, pos):
        ...
        # arrow if sphere is above surface
            ...
            pygame.draw.aaline(surface, (0, 0, 0), (centerx + 50, 50), (centerx, 20))
        pygame.draw.aaline(surface, (0, 0, 0), (width - 5, height * 0.3),
                           (width - 5, (height * 0.3) + (calculations[2] * scale)))
        pygame.draw.aaline(surface, (0, 0, 0), (width - 5, height * 0.3),
                           (width - 5, (height * 0.3) + (calculations[2] * scale - _height)))
        return surface
    ...
    def forces(self, surface, calculations, radius, scale, pos):
        ...
        (surface.get_width() / 2) + (scale * 0.1), pos + (scale), -scale * 0.2, -arrowHeight_buoyancy))
        ...
        pygame.draw.rect(surface, BLUE,
                         ((surface.get_width() / 2) + (scale * 0.1), pos + (scale), -scale * 0.2, arrowHeight_gravity))
...
class Window:
    def __init__(self):
        ...
        print
        "Finished initiating Pygame_handling module..."
    ...
    def create_cb(self, text, pos):
        for n in range(len(text)):
            cb = zCheckbutton(text[n], pos[n], nothing, nothing, 2, os.path.join(self.Draw.font_path, 'CALIBRI.TTF'),
                              BLACK, BLACK, WHITE)
            self.cbs.append(cb)
