...
try:
    ...
    print
    "zModule: warning, PyIgnition not found, will crash if you are trying to add an effect"
    ...
    def __init__(self, x=800, y=600, title="pygame window", icon=None, titlepos=None,
        ...
        try:
            ...
            if isinstance(effect, ParticleEffect):
                ...
                print
                "Warning, the effect is not valid, defaulting to NoEffect"
...
class zCheckbutton:
    ...
    def __init__(self, text, pos, activate, deactivate, m=2, font=None, textcolor=BLACK, rectcolor=WHITE,
                 crosscolor=BLACK, state=False):
        if not (text and m):
        ...
        pygame.draw.polygon(self.selected_image, crosscolor,
                            ((1.5 * m, 0), (0, 1.5 * m), (8.5 * m, 10 * m), (10 * m, 8.5 * m)))
        pygame.draw.polygon(self.selected_image, crosscolor,
                            ((8.5 * m, 0), (10 * m, 1.5 * m), (1.5 * m, 10 * m), (0, 8.5 * m)))
...
class zTextImgButton(zButton):
    ...
    def __init__(self, function, pos, image_path, text, dim, font=None, color=BLACK, prcolor=BLACK, primage_path=None,
                 colorkey=None):
        font = pygame.font.Font(font, dim)
...
class zNumManager:
    ...
    def __init__(self, text, pos, defaultvalue=0, minvalue=None, maxvalue=None, dim=2, font=None, clr=BLACK,
                 arrowclr=BLACK, prarrowclr=WHITE):
        ...
        self.uparrow = zButton(self.valueup, (self.pos[0], self.pos[1] - self.dim * 2 / 3), normal_arrow.copy(),
                               pressed_arrow.copy())
        self.downarrow = zButton(self.valuedown, (self.pos[0], self.pos[1] + self.dim * 2 / 3),
                                 pygame.transform.rotate(normal_arrow, 180),
                                 pygame.transform.rotate(pressed_arrow, 180))
