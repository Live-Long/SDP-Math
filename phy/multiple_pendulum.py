...
class MyApp(Tk):
    def __init__(self):
        ...
        for i in np.arange(len(self.p)):
            self.p[i].cv_lin = self.canvas.create_line(self.fix[0], self.fix[1], self.p[i].pos[0], self.p[i].pos[1],
                                                       width=1, fill=colorT((100, 100, 100)))
            self.p[i].cv_dot = self.canvas.create_oval(self.p[i].pos[0] - self.r, self.p[i].pos[1] - self.r,
                                                       self.p[i].pos[0] + self.r, self.p[i].pos[1] + self.r)
    ...
    def update(self):
        ...
        for p_ in self.p:
            ...
            self.canvas.coords(p_.cv_dot, p_.pos[0] - self.r, p_.pos[1] - self.r, p_.pos[0] + self.r,
                               p_.pos[1] + self.r)
